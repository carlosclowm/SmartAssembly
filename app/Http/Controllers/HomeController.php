...
class HomeController extends Controller
{
    ...
    public function getResults(Request $res){
        //{\"cheap\":false, \"gamer\":true, \"intel\":true}
        $json = "{";
        if($res->get('cheap')){
            $json = $json."\"cheap\":true,";
        }else{
            $json = $json."\"cheap\":false,";
        }
        if($res->get('games')){
            $json = $json."\"gamer\":true,";
        }else{
            $json = $json."\"gamer\":false,";
        }
        if($res->get('intel')){
            $json = $json."\"intel\":true";
        }else{
            $json = $json."\"intel\":false";
        }
        $json = $json."}";
        //Area Results
        $command = escapeshellcmd('python C:\MyRuleEngine.py "'.$json.'" ');
    ...
    public function startScript($json){
        $command = escapeshellcmd('python C:\MyRuleEngine.py "'.$json.'" ');
        $output = shell_exec($command);
        $result = preg_replace('/\s+/', '+', $output);
        $curl = curl_init();
